    ...
    fun controlsStayWithinSupportedRanges() {
        PresetCatalog.presets.forEach { preset ->
            with(preset.adjustments) {
                ...
                listOf(fade, vignette, grain, sharpen, pixelStretch, pixelStretchPosition, shadowTone, highlightTone)
                    .forEach { assertTrue("${preset.id}: effect", it in 0f..1f) }
                assertTrue("${preset.id}: pixel stretch band", pixelStretchBand in 0.01f..0.30f)
            }
        }
    }
}
