...
object ImageProcessor {
    suspend fun apply(
        ...
        val sharpened = adjustments.sharpen > 0f && mix > 0f && width > 2 && height > 2
        val gradedPixels = if (sharpened) {
            sharpen(processedPixels, sourcePixels, width, height, adjustments.sharpen * mix)
            sourcePixels
        } else {
            processedPixels
        }
        val stretchWorkspace = if (sharpened) processedPixels else sourcePixels
        val finalPixels = if (adjustments.pixelStretch > 0f && mix > 0f) {
            pixelStretch(
                input = gradedPixels,
                output = stretchWorkspace,
                width = width,
                height = height,
                amount = adjustments.pixelStretch * mix,
                position = adjustments.pixelStretchPosition,
                band = adjustments.pixelStretchBand,
                direction = adjustments.pixelStretchDirection,
            )
            stretchWorkspace
        } else {
            gradedPixels
    ...
    /**
     * Expands a narrow row or column into a larger region while keeping the canvas size fixed.
     * The untouched pixels remain in place, and a short feather prevents hard seams at partial
     * stretch amounts.
     */
    private suspend fun pixelStretch(
        input: IntArray,
        output: IntArray,
        width: Int,
        height: Int,
        ...
        position: Float,
        band: Float,
        direction: PixelStretchDirection,
    ) {
        input.copyInto(output)
        val horizontal = direction == PixelStretchDirection.Horizontal
        val axisSize = if (horizontal) width else height
        if (axisSize < 2) return

        val safeAmount = amount.coerceIn(0f, 1f)
        val sampleSize = (axisSize * band.coerceIn(0.005f, 0.30f))
            .roundToInt()
            .coerceIn(1, axisSize)
        val center = ((axisSize - 1) * position.coerceIn(0f, 1f)).roundToInt()
        val sampleStart = (center - sampleSize / 2).coerceIn(0, axisSize - sampleSize)
        val sampleEnd = sampleStart + sampleSize - 1
        val targetStart = (sampleStart * (1f - safeAmount)).roundToInt()
        val targetEnd = (sampleEnd + (axisSize - 1 - sampleEnd) * safeAmount).roundToInt()
        val targetSpan = (targetEnd - targetStart).coerceAtLeast(1)
        val sampleSpan = (sampleEnd - sampleStart).coerceAtLeast(0)
        val feather = (axisSize * 0.012f).roundToInt()
            .coerceAtLeast(1)
            .coerceAtMost((targetSpan / 4).coerceAtLeast(1))

        for (axis in targetStart..targetEnd) {
            if ((axis - targetStart) % 32 == 0) currentCoroutineContext().ensureActive()
            val normalized = (axis - targetStart) / targetSpan.toFloat()
            val sourceAxis = sampleStart + (normalized * sampleSpan).roundToInt()
            var blend = 1f
            if (targetStart > 0 && axis < targetStart + feather) {
                blend *= smoothstep(targetStart.toFloat(), (targetStart + feather).toFloat(), axis.toFloat())
            }
            if (targetEnd < axisSize - 1 && axis > targetEnd - feather) {
                blend *= smoothstep(targetEnd.toFloat(), (targetEnd - feather).toFloat(), axis.toFloat())
            }

            if (horizontal) {
                for (y in 0 until height) {
                    val targetIndex = y * width + axis
                    val sampleIndex = y * width + sourceAxis
                    output[targetIndex] = blendColor(input[targetIndex], input[sampleIndex], blend)
                }
            } else {
                val targetRow = axis * width
                val sampleRow = sourceAxis * width
                for (x in 0 until width) {
                    output[targetRow + x] = blendColor(input[targetRow + x], input[sampleRow + x], blend)
                }
            }
        }
    }

    private fun blendColor(original: Int, stretched: Int, amount: Float): Int {
        if (amount >= 0.999f) return stretched
        return Color.argb(
            lerp(Color.alpha(original).toFloat(), Color.alpha(stretched).toFloat(), amount).roundToInt().coerceIn(0, 255),
            lerp(Color.red(original).toFloat(), Color.red(stretched).toFloat(), amount).roundToInt().coerceIn(0, 255),
            lerp(Color.green(original).toFloat(), Color.green(stretched).toFloat(), amount).roundToInt().coerceIn(0, 255),
            lerp(Color.blue(original).toFloat(), Color.blue(stretched).toFloat(), amount).roundToInt().coerceIn(0, 255),
        )
    }
